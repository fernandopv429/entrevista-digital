...
import { CheckCircle2, FileText, Loader2, ListChecks } from "lucide-react";
...
export default function Home() {
  ...
  const submit = async (event) => { event.preventDefault(); setSaving(true); await base44.entities.Entrevista.create(data); try { await base44.functions.invoke('enviarWebhookEntrevista', data); } catch (e) { /* webhook failure não impede o salvamento */ } setSaving(false); setSaved(true); window.scrollTo({ top: 0, behavior: "smooth" }); };

  return <main className="min-h-screen bg-slate-50 pb-20 text-slate-950"><header className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12"><div className="mb-6 flex items-center gap-3"><div className="rounded-xl bg-slate-950 p-2.5 text-white"><FileText className="h-5 w-5" /></div><div><p className="font-bold tracking-[0.18em]">FERNANDO VIEIRA</p><p className="text-xs tracking-[0.35em] text-slate-500">ADVOGADOS</p></div></div><p className="mb-2 text-sm font-bold uppercase tracking-widest text-blue-700">Entrevista trabalhista</p><h1 className="max-w-2xl text-3xl font-bold tracking-tight sm:text-4xl">Formulário de atendimento ao cliente</h1><p className="mt-3 max-w-2xl text-slate-600">Revise os dados extraídos do documento e atualize qualquer informação necessária antes de salvar.</p><div className="mt-5"><Link to="/entrevistas" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"><ListChecks className="h-4 w-4" />Ver entrevistas salvas</Link></div>{saved && <div className="mt-6 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800"><CheckCircle2 className="h-5 w-5" />Entrevista salva com sucesso.</div>}</div></header>
